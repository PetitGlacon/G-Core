package fr.the__glacier.gCoreV.util;
...
import fr.the__glacier.gCoreV.GCoreV;
...
public class ConfigurationManager {
    ...
    private final GCoreV javaPlugin;
     ...
     */
    public ConfigurationManager(PersistType persistType, GCoreV javaPlugin) {
    ...
    public void saveFile(Object instance, File file) {
        try {
            ...
            javaPlugin.getLogger().error("Failed to save {}: {}", file.toString(), e.getMessage());
        }
    }
    ...
    public String toString(Object instance) {
        try {
            ...
            javaPlugin.getLogger().error("Failed to save {}: {}", instance.toString(), e.getMessage());
        }
        return "";
    }
    ...
    public <T> T load(Class<T> clazz, File file) {
        if (file.exists()) {
            try {
                ...
                javaPlugin.getLogger().error("Failed to parse {}: {}", file, e.getMessage());
                javaPlugin.getLogger().error("Creating a backup for {} in \"backups\" folder...", file.getName());
                ...
                try {
                    ...
                    javaPlugin.getLogger().info("Success! Backup \"{}\" created, check \"{}\".", file.getName(), backupFolder.getPath());
                } catch (IOException exception) {
                    javaPlugin.getLogger().error("Failed to move {} to {}{}backups: {}", file, javaPlugin.getDataFolder().getName(), File.separator, exception.getMessage());
                    javaPlugin.getLogger().error(e.getMessage());
                }
                load(clazz, file);
            }
        }
